...
static void CreateCubeRT()
{
    ...
    if (FAILED(g_pDevice->CreateCubeTexture(
        CUBE_FACE_SZ, 1, D3DUSAGE_RENDERTARGET,
        D3DFMT_X8R8G8B8, D3DPOOL_DEFAULT, &s_cubeMap)))
    {
        s_cubeMap = NULL;
        return;
    }
    ...
    if (FAILED(g_pDevice->CreateDepthStencilSurface(
        CUBE_FACE_SZ, CUBE_FACE_SZ, D3DFMT_D16,
        D3DMULTISAMPLE_NONE, &s_cubeDepth)))
    {
        s_cubeMap->Release();
        s_cubeMap = NULL;
        return;
...
void CubeEnvScene_Render(float)
{
    ...
    if (s_cubeReady)
    {
        int phCam = (int)((tMs / 9u) & 1023u);
        int phCamY = (int)((tMs / 23u) & 1023u);
        float camX = s_cos[phCam] * 6.0f;
        float camY = 1.4f + s_sin[phCamY] * 0.7f;
        float camZ = s_sin[phCam] * 6.0f;
        float cl = sqrtf(camX * camX + camY * camY + camZ * camZ);
        if (cl > 0.0001f) { camX /= cl; camY /= cl; camZ /= cl; }

        for (int attempt = 0; attempt < 6; ++attempt)
        {
            int face = s_nextFace;
            s_nextFace = (s_nextFace + 1) % 6;
            if (IsFaceVisible(face, world, camX, camY, camZ))
            {
                RenderCubeFace(face, tMs);
                break;
            }
        }
    }
